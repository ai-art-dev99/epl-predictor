...
import { predictMatch, getTeams } from '../utils/api';
...
export default function PredictorPage() {
  ...
  const handlePredict = async () => {
    ...
    setResult(null);
    setAgentSteps([
      { step: 'Fetching EPL standings...', done: false },
      { step: `Analyzing ${homeTeam} recent form...`, done: false },
      { step: `Analyzing ${awayTeam} recent form...`, done: false },
      { step: 'Checking head-to-head history...', done: false },
      { step: 'Running AI analysis...', done: false },
    ]);

    // Simulate step progression during loading
    const stepTimers = [1200, 2800, 4400, 6000, 7500].map((delay, i) =>
      setTimeout(() => {
        setAgentSteps(prev => prev.map((s, idx) => idx === i ? { ...s, done: true } : s));
      }, delay)
    );
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Prediction failed. Please check your API keys and try again.');
    } finally {
      setLoading(false);
  ...
  const quickPickMatch = (home, away) => {
    setHomeTeam(home);
    setAwayTeam(away);
    setResult(null);
  };
    ...
    <div style={{ animation: 'slide-up 0.5s ease' }}>
      ...
      <div style={{ marginBottom: 40 }}>
        <h1 style={{
          ...
          <span style={{
            background: 'linear-gradient(90deg, #3d9fff, #00e676)',
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 16, alignItems: 'end', marginBottom: 24 }}>
          ...
          <div>
            ...
            <select
              value={homeTeam}
              onChange={e => { setHomeTeam(e.target.value); setResult(null); setError(''); }}
              style={selectStyle}
            >
          ...
          <div>
            ...
            <select
              value={awayTeam}
              onChange={e => { setAwayTeam(e.target.value); setResult(null); setError(''); }}
              style={selectStyle}
            >
              <option value="">Select team...</option>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 10 }}>
            QUICK PICKS
          </div>
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '16px',
            background: loading || !homeTeam || !awayTeam
              ? 'var(--bg-secondary)'
              : 'linear-gradient(135deg, #1a6fd4, #0ba360)',
            border: 'none',
            borderRadius: 10,
            ...
            {agentSteps.map((step, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  {step.done ? '✓' : <span style={{ animation: i === agentSteps.findIndex(s => !s.done) ? 'pulse-glow 1s infinite' : 'none', color: 'var(--accent-blue)' }}>●</span>}
                </div>
                <span style={{
                  fontSize: 13,
