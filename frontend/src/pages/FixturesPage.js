...
import { getUpcomingFixtures, predictMatch } from '../utils/api';
...
export default function FixturesPage() {
  ...
  useEffect(() => {
    getUpcomingFixtures()
      ...
      .catch(e => setError(e.response?.data?.detail || 'Failed to load fixtures'))
  ...
  return (
    <div style={{ animation: 'slide-up 0.5s ease' }}>
      ...
      {error && (
        <div style={{ background: 'rgba(255,71,87,0.1)', border: '1px solid rgba(255,71,87,0.3)', borderRadius: 12, padding: 24, color: 'var(--accent-red)', marginBottom: 24 }}>
          {error}
        </div>
      )}
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)' }}>
                        MD {match.matchday} · {match.match_date?.split('T')[1]?.slice(0, 5) || ''}
                      </span>

                      {isLoading ? (
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--accent-blue)', animation: 'pulse-glow 1s infinite' }}>
                          ANALYZING...
                        </span>
                      ) : result?.error ? (
                  ...
                  {result && !result.error && (
                    <div style={{
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 15, fontWeight: 700, color: 'var(--accent-blue)' }}>
...
function formatDate(dateStr) {
  ...
  try {
    ...
    return d.toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }).toUpperCase();
  } catch {
    return dateStr;
  }
}
