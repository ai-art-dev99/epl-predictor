...
const OUTCOME_COLORS = { home_win: '#3d9fff', away_win: '#00e676', draw: '#ffb300' }
  ...
  return (
    <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 700, color }}>{pct}%</span>
      </div>
    </div>
  )
}
...
export default function PredictionResult({ result }) {
  ...
  return (
    <div style={{ animation: 'slide-up 0.5s ease' }}>
      {/* Verdict */}
      <div style={{
        background: 'var(--bg-card)', border: `1px solid ${outcomeColor}40`,
        borderRadius: 16, padding: 32, marginBottom: 24, position: 'relative', overflow: 'hidden',
      }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 20 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 28, fontWeight: 700, color: outcomeColor, minWidth: 70, textAlign: 'center', letterSpacing: 2 }}>
            ...
            <div style={{
              ...
              background: `${outcomeColor}18`, border: `1px solid ${outcomeColor}50`,
              borderRadius: 6, padding: '6px 14px', fontFamily: 'var(--font-display)',
              fontWeight: 700, fontSize: 13, color: outcomeColor, letterSpacing: '0.08em',
            }}>
              ⚡ {OUTCOME_LABELS[result.predicted_outcome]}
            </div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 8 }}>CONFIDENCE</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
          {[
            ...
            <div key={key} style={{
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 28, fontWeight: 700, color }}>{prob}%</div>
            </div>
          ))}
        </div>
      ...
      {result.key_factors?.length > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, marginBottom: 24 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 16 }}>KEY FACTORS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {result.key_factors.map((factor, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <span style={{
                  width: 22, height: 22, borderRadius: 4, background: 'rgba(61,159,255,0.15)',
                  border: '1px solid rgba(61,159,255,0.3)', display: 'flex', alignItems: 'center',
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 16 }}>AI ANALYSIS</div>
        <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.8, whiteSpace: 'pre-wrap', borderLeft: '2px solid var(--border-bright)', paddingLeft: 16 }}>
          {result.analysis}
        </div>
      </div>
    </div>
